...
const enable_allow_streaming = computed(() => {
  ...
  // 'ng' engine owns streaming — legacy Streaming.vue stays mounted only in legacy mode
  if (global_settings.settings.render.engine === 'ng') {
    return false;
  }
...
const runtimes = usesManagedChatSurface
  ...
  : global_settings.settings.render.engine === 'legacy'
