...
// `version` is the reactivity channel — the map is shallow (see store)
const iframe_ids = computed(() => {
  void store.version;
  return [...store.iframe_logs.keys()].toSorted();
});
...
const logs = computed(() => {
  void store.version; // shallow map — recompute on throttled version tick
