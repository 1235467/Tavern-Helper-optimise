...
// the 1.2MB editor bundle is lazy-loaded in onMounted — keep the type +
// the two string-enum values locally so nothing else needs the import
import type { Content, JSONEditorPropsOptional, Mode, ValidationSeverity } from 'vanilla-jsoneditor';

const Mode = { text: 'text', tree: 'tree', table: 'table' } as const;
const ValidationSeverity = { error: 'error' } as const;
type JSONEditorInstance = Awaited<ReturnType<typeof import('vanilla-jsoneditor').createJSONEditor>>;
...
let editor_instance: JSONEditorInstance;
...
onMounted(async () => {
  ...
  if (is_unmounted) {
    return; // component unmounted while the 1.2MB bundle loaded
  }
